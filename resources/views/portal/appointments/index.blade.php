    ...
    <div class="container mx-auto px-6 py-12 max-w-3xl">
        ...
        <div class="mt-3 space-y-3">
            @forelse ($upcoming as $appt)
                <div class="rounded-2xl bg-white border border-slate-200/60 p-5 shadow-soft flex items-center justify-between gap-4">
                    <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @if ($appt->isCancellable())
                            <form method="POST" action="{{ route('portal.appointments.cancel', $appt) }}" onsubmit="return confirm('Cancel this appointment?');">
                                @csrf
        ...
        <div class="mt-3 space-y-2">
            @forelse ($past as $appt)
                <div class="flex items-center justify-between text-sm border-b border-slate-100 py-2">
                    <div>{{ $appt->scheduled_at->format('M j, Y') }} · {{ $appt->service?->name ?? '—' }} · {{ $appt->dentist?->name }}</div>
                    <span class="px-2.5 py-0.5 rounded-full text-xs font-medium {{ $appt->status->badgeClasses() }}">{{ $appt->status->label() }}</span>
                </div>
            @empty
