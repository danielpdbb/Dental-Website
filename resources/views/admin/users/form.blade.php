...
@php($isPatientEdit = $isEdit && optional($user)->role?->value === 'patient')
...
@if ($isPatientEdit)
    {{-- Patients manage their own details; the clinic may only change account status. --}}
    <div class="mb-5 rounded-xl border border-slate-200 bg-slate-50 text-slate-600 px-4 py-3 text-sm">
        Patient details and password are managed by the patient in their own profile.
        Here you can only change their <strong>account status</strong>.
    </div>

    <div class="grid sm:grid-cols-2 gap-5">
        <div>
            <span class="block text-sm font-medium text-slate-700 mb-1">Full name</span>
            <div class="h-11 px-4 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 flex items-center">{{ $user->name }}</div>
        </div>
        <div>
            <span class="block text-sm font-medium text-slate-700 mb-1">Username</span>
            <div class="h-11 px-4 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 flex items-center">{{ $user->username }}</div>
        </div>
        <div>
            <span class="block text-sm font-medium text-slate-700 mb-1">Email</span>
            <div class="h-11 px-4 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 flex items-center">{{ $user->email }}</div>
        </div>
        <div>
            <span class="block text-sm font-medium text-slate-700 mb-1">Role</span>
            <div class="h-11 px-4 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 flex items-center">{{ $user->role->label() }}</div>
        </div>
        <div>
